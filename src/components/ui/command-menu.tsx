...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn(
      "text-lg font-semibold leading-none tracking-tight text-[#121722]",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description
    ref={ref}
    className={cn("text-sm text-[#777c86]", className)}
    {...props}
  />
));
        ...
        <DialogPrimitive.Content asChild ref={ref} {...props}>
          ...
          >
            ...
            >
              ...
              <CommandMenuClose className="absolute right-3 top-3 rounded-lg p-1.5 text-[#777c86] hover:text-[#121722] hover:bg-[#faf9f7] focus-visible:outline-none transition-colors cursor-pointer">
                <X size={14} />
...
>(
  (
    { className, placeholder = "Search positions, companies...", ...props },
    ref
  ) => {
    const { value, setValue } = useCommandMenu();

    return (
      <div className="flex items-center border-b border-[#efefef] px-4 py-0">
        <Search className="mr-3 h-4 w-4 shrink-0 text-[#777c86]" />
        <input
          ref={ref}
          value={value}
          onChange={(e) => setValue(e.target.value)}
          className={cn(
            "flex h-14 w-full rounded-none border-0 bg-transparent py-3 text-sm text-[#121722] outline-none placeholder:text-[#777c86] disabled:cursor-not-allowed disabled:opacity-50 font-medium",
            className
          )}
...
>(({ className, children, maxHeight = "350px", ...props }, ref) => {
  ...
  return (
    <div ref={ref} className="p-2" {...props}>
      <ScrollArea
        className={cn("w-full scrollbar-thin", className)}
...
>(({ className, children, heading, ...props }, ref) => (
  <div ref={ref} className={cn("py-1", className)} {...props}>
    {heading && (
      <div className="px-3 py-2 text-[11px] font-bold text-[#777c86] uppercase tracking-wider">
        {heading}
      </div>
    )}
    {children}
  ...
  (
    ...
    return (
      <div
        ...
        data-command-item
        className={cn(
          "relative flex cursor-pointer select-none items-center rounded-xl px-3 py-2.5 text-sm outline-none transition-colors gap-3",
          "hover:bg-[#faf9f7] text-[#121722]",
          isSelected &&
            "bg-[#faf9f7] text-[#0068f9] font-medium",
      ...
      >
        {icon && (
          <div className={cn("h-4 w-4 flex items-center justify-center", isSelected ? "text-[#0068f9]" : "text-[#777c86]")}>{icon}</div>
        )}
          ...
          <div className="ml-auto flex items-center gap-1">
            {shortcut.split("+").map((key, i) => (
              <React.Fragment key={key}>
                {i > 0 && (
                  <span className="text-[#777c86] text-xs">
                    +
                  </span>
                )}
                <Kbd size="xs">
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn("-mx-1 my-1 h-px bg-[#efefef]", className)}
    {...props}
  />
));
