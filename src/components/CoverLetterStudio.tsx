...
import { X, Printer, Copy, Check, FileText, Plus, Trash2, Loader2 } from 'lucide-react';
...
export function CoverLetterStudio({ 
  ...
  const fallbackKey = `studio_cover_letter_${(companyName || 'general').replace(/\s+/g, '_')}_${(_targetRole || 'general').replace(/\s+/g, '_')}`;
  ...
  const triggerAutoSave = useCallback(() => {
    ...
    try {
      ...
      console.warn('CoverLetterStudio auto-save to localStorage failed:', e);
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      const savedStyle = localStorage.getItem(`${effectiveStorageKey}_style`) || (fallbackKey !== effectiveStorageKey ? localStorage.getItem(`${fallbackKey}_style`) : null);
      if (savedStyle && ['classic', 'modern', 'executive', 'academic'].includes(savedStyle)) {
        setStyle(savedStyle as CoverLetterStyle);
      }
    } catch (e) {}
  ...
  }, [effectiveStorageKey, style, pages]);
  ...
  const handlePrint = () => {
    ...
    if (style === 'classic') {
      fontStyle = "font-family: 'Times New Roman', Georgia, serif; font-size: 11pt; line-height: 1.6; color: #111;";
    } else if (style === 'executive') {
      fontStyle = "font-family: 'Georgia', serif; font-size: 10.5pt; line-height: 1.65; color: #0f172a;";
        ...
        <head>
          ...
          <style>
            ...
            * { box-sizing: border-box; }
            ...
            .content h1 { font-size: 18pt; font-weight: bold; margin: 0.8em 0 0.3em; }
            .content h2 { font-size: 14pt; font-weight: bold; margin: 0.7em 0 0.3em; }
            .content h3 { font-size: 12pt; font-weight: bold; margin: 0.6em 0 0.25em; }
    ...
    <div className={`bg-[#faf9f7] rounded-2xl w-full flex flex-col overflow-hidden border border-[#efefef] ${
      ...
      <div className="flex flex-wrap items-center justify-between px-4 sm:px-6 py-4 bg-white border-b border-[#efefef] shrink-0 gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 rounded-xl bg-[#eef5ff] text-[#0068f9] flex items-center justify-center font-bold shrink-0">
            <FileText size={20} />
          </div>
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <h2 className="text-base sm:text-lg font-bold text-[#121722] truncate">Cover Letter Studio</h2>
            ...
            <p className="text-xs text-[#777c86] flex items-center gap-1.5 flex-wrap">
              <span className="truncate max-w-[220px] sm:max-w-none">{companyName ? `Tailored for ${companyName}` : 'Custom cover letter'}</span>
              <span>•</span>
          ...
          >
            ...
            <div className="shrink-0 flex items-center justify-between text-[11px] text-slate-400 font-medium pb-3 mb-3 border-b border-slate-100 select-none">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="bg-[#f0f4ff] text-[#0068f9] font-bold px-2.5 py-0.5 rounded-full text-[10px]">
                  Page {idx + 1}
                </span>
              </div>
            </div>
